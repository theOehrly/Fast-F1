...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "ergastapi: test connects to the ergast api"
    )
    config.addinivalue_line(
        "markers",
...
def pytest_collection_modifyitems(config, items):
    ...
    if config.getoption("--prj-doc"):
        skip_non_prj = pytest.mark.skip(
            reason="--prj-doc given: run only "
        ...
        skip_prj = pytest.mark.skip(
            reason="need --prj-doc to run project "
...
def pytest_report_teststatus(report, config):
    ...
    if (report.when == "teardown") and (Cache._request_counter > 0):
        name = f"{report.location[0]}::{report.location[2]}"
        line = f"{name} - uncached requests:\t {Cache._request_counter}"
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    if content:
        ...
        terminalreporter.section(
            "Request count", sep="-", blue=True, bold=True
        )
        terminalreporter.line(content)

    terminalreporter.ensure_newline()
    terminalreporter.section(
        "Parameter Overrides", sep="-", blue=True, bold=True
    )
    terminalreporter.line(
        f"Ergast backend override: {ERGAST_BACKEND_OVERRIDE}"
    )

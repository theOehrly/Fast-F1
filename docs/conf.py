...
warnings.filterwarnings(
    action="ignore", message=r"`fastf1.api` will be considered private .*"
)
warnings.filterwarnings(
    action="ignore",
    message=r"`utils.delta_time` is considered " r"deprecated.*",
)
warnings.filterwarnings(
    action="ignore",
...
exclude_patterns = ["../fastf1/signalr_aio"]
...
sphinx_gallery_conf = {
    ...
    "image_scrapers": (
        "matplotlib",  # default
        plotly_sg_scraper,
    ),  # for plotly thumbnail
    "reset_modules": (
        "matplotlib",
        "seaborn",  # defaults
        sphinx_gallery_setup,
    ),  # custom setup
